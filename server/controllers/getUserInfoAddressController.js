...
const getUserInfoAndAddress = async (req, res) => {
    try {
        ...
        const userInfoAndAddressObject = await Address.findOne({ "userInfo._id": req.params.id });
        const userInfoObject = await User.findOne({ "_id": ObjectId(req.params.id) });

        console.log(userInfoAndAddressObject);
        console.log(userInfoObject);
        if (!userInfoAndAddressObject || !userInfoObject) {
            return res.status(404).json({ error: 'User not found with the provided address ID' });
        }
